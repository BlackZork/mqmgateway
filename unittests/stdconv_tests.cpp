...
TEST_CASE("A number value should be divided") {
    ...
    SECTION("when one int32 value is converted to two modbus registers") {
    ...
    SECTION("when two modbus registers are converted into a string") {
        ModbusRegisters input = ModbusRegisters(std::vector<uint16_t>{0x04, 0x9B4B});
        MqttValue output = conv->toMqtt(input);

        REQUIRE(strcmp(output.getString().data(), "150949.5") == 0);
    }
}
