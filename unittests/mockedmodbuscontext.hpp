...
class MockedModbusContext : public modmqttd::IModbusContext {
    public:
        ...
        virtual uint16_t waitForModbusValue(int slaveId, int regNum, modmqttd::RegisterType regType, uint16_t val, std::chrono::milliseconds timeout = defaultWaitTime());
        virtual uint16_t getModbusRegisterValue(int slaveId, int regNum, modmqttd::RegisterType regtype);
        virtual void waitForInitialPoll(std::chrono::milliseconds timeout = defaultWaitTime());
...
class MockedModbusFactory : public modmqttd::IModbusFactory {
    public:
        ...
        uint16_t waitForModbusValue(const char* network, int slaveId, int regNum, modmqttd::RegisterType regType, uint16_t val, std::chrono::milliseconds timeout = defaultWaitTime());
    private:
